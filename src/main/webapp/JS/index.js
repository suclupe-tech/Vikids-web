...
document.addEventListener("DOMContentLoaded", () => {
    ...
    window.enviarPedidoPorWhatsApp = function () {
        const numeroWhatsApp = '5198765432'; // Cambia por tu número real
        if (cart.length === 0) {
        ...
        cart.forEach((item) => {
            mensaje += `- ${item.name} (${item.quantity} x S/ ${item.price.toFixed(2)})\n`;
        });
        const total = cart.reduce((sum, item) => sum + (item.price * item.quantity), 0);
        mensaje += `\nTotal: S/ ${total.toFixed(2)}`;
        // Codifica correctamente el mensaje
